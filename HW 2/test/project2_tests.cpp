...
int main()
{
	...
	WAVLTree<double, char> tree;
	std::vector<std::pair <double, char> > data = {{4.0, 'a'}, {5.0, 'b'}, {2.0, 'c'}, {1.0, 'd'}}; 
	...
	tree.insert(0.0, 'e'); // single rotation
	...
	// add new tests
	tree.insert(0.5, 'f');
	std::cout << "getSize(): " << tree.getSize() << ", Expected: 6" << std::endl;
	std::cout << "getHeight(): " << tree.getHeight() << ", Expected: 3" << std::endl;
	std::cout << "getRank(2): " << tree.getRank(2) << ", Expected: 1" << std::endl;
	std::cout << "getRank(1): " << tree.getRank(1) << ", Expected: 3" << std::endl;
	std::cout << "getRank(0): " << tree.getRank(0) << ", Expected: 2" << std::endl;
	std::cout << "getRank(0.5): " << tree.getRank(0.5) << ", Expected: 1" << std::endl;
	std::cout << "find(0.5): " << tree.find(0.5) << ", Expected: f" << std::endl;
	tree.printTree();
	std::cout << "----------------------" << std::endl;
	//Test insert double rotation
	tree.insert(0.3, 'f');
	tree.printTree();
	std::cout << "----------------------" << std::endl;
	//Test general BST delete
	tree.remove(0.3);
	tree.printTree();
	std::cout << "----------------------" << std::endl;
	//Test delete Case when the childs of a 2-2 node are all leaves
	tree.remove(0);
	tree.printTree();
	std::cout << "----------------------" << std::endl;
	//Test delete Case 2b
	...
	//Test using maxVal
	WAVLTree<int, double> tree2(true);
	tree2.insert(1, 0.3);
	tree2.insert(2, 0.4);
	tree2.insert(3, 0.32);
	tree2.insert(4, 0.45);
	...
	tree2.insert(8, 0.48);
	tree2.printTree();
	std::cout << "best bin for 0.31 is " << tree2.findByMaxVal(0.31) << std::endl;
	std::cout << "best bin for 0.48 is " << tree2.findByMaxVal(0.48) << std::endl;
	std::cout << "best bin for 0.45 is " << tree2.findByMaxVal(0.45) << std::endl;
	std::cout << "best bin for 0.4 is " << tree2.findByMaxVal(0.4) << std::endl;
	std::cout << "best bin for 0.5 is " << tree2.findByMaxVal(0.5) << std::endl;
	std::cout << "----------------------" << std::endl;

	//Test insert duplicate
	WAVLTree<int, double> tree3;
	tree3.insert(4.0, 'f');
	tree3.insert(4.0, 'm');
	tree3.insert(4.0, 'p');
	tree3.insert(4.0, '5');
	tree3.insert(4.0, '2');
	tree3.insert(4.0, '0');
	std::cout << "getSize(): " << tree3.getSize() << ", Expected: 6" << std::endl;
	std::cout << "getHeight(): " << tree3.getHeight() << ", Expected: 3" << std::endl;
	tree3.printTree();
	std::cout << "----------------------" << std::endl;
